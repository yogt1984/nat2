...
def ledger_with(tmp_path, *, prereg=True, map_verdict="pass"):
...
def test_refuses_below_event_or_day_floor(tmp_path, monkeypatch, scored, days):
    ...
    assert v.detail["reason"] == "insufficient_forward_events" and v.detail["judged_against"] == [0]
...
def cell(coin="BTC", h="1h", k=1.0, beats=True, p=0.001, hit=0.62, thr=0.55):
...
def test_decide_known_positive_and_negative():
    ...
    assert d["would_pass_on_1_3_4"] and d["criteria"]["2_alpha_kernel"] == "not_evaluable" and not d["passed"]
...
def test_runnable_path_records_cells_and_provenance(tmp_path, monkeypatch):
    ...
    assert v.detail["verdict"] == "fail" and v.detail["would_pass_on_1_3_4"] and v.detail["universe"] == ["BTC"]
